...
library(tidyverse) # For dplyr (pipe operations and feature engineering) and ggplot2 (visualizations)
...
In preparation for building the predictive model, I choose to join in the city of the search by using `Location_Code` and removing the now-unneeded fields `URL`, `Description`, `Description_Upper`, `Job_Title`, and `Description`. Because I don't necessarily want to remove `DSdata` from memory, I will rename this new dataframe as `data`:
